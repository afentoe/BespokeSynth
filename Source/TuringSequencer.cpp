...
//
//
//  Created by Jack van Klaren on Aug 24 2024.
//  Based on NoteStepSequencer by Ryan Challinor
//
//

#include "TuringSequencer.h"
...
TuringSequencer::TuringSequencer()
{
   for (int i = 0; i < TM_MAX_STEPS; ++i)
   {
...
void TuringSequencer::CreateUIControls()
{
   ...
   FLOATSLIDER(mRandomizeStepChanceSlider, "rand chance", &mRandomizeStepChance, 0, 1);
   UIBLOCK_SHIFTRIGHT();
   UIBLOCK_SHIFTX(8);
   UICONTROL_CUSTOM(mGridControlTarget, new GridControlTarget(UICONTROL_BASICS("grid")));
   UIBLOCK_SHIFTRIGHT();
   UIBLOCK_PUSHSLIDERWIDTH(48);
   ...
   INTSLIDER(mLengthSlider, "length", &mLength, 1, TM_MAX_STEPS);
   UIBLOCK_SHIFTRIGHT();
   UIBLOCK_SHIFTX(2);
   UIBLOCK_PUSHSLIDERWIDTH(48);
   FLOATSLIDER_DIGITS(mOctaveLoSlider, "oct lo", &mOctaveLo, 0, 5, 0);
   UIBLOCK_SHIFTRIGHT();
   UIBLOCK_SHIFTX(2);
   UIBLOCK_PUSHSLIDERWIDTH(48);
   FLOATSLIDER_DIGITS(mOctaveHiSlider, "oct hi", &mOctaveHi, 0, 5, 0);
   UIBLOCK_SHIFTRIGHT();
   BUTTON(mSetStepToZeroButton, "0");
   UIBLOCK_SHIFTRIGHT();
   UIBLOCK_SHIFTX(2);
   BUTTON(mSetStepToOneButton, "1");
   UIBLOCK_SHIFTRIGHT();
   UIBLOCK_SHIFTX(6);
   ...
   mGrid = new UIGrid("notegrid", 5, 40, 320, 20, 16, 1, this);
...
//   mGrid->SetClickValueSubdivisions(mStepLengthSubdivisions);
   ...
   mAllNotesCable = new AdditionalNoteCable();
   mAllNotesCable->SetPatchCableSource(new PatchCableSource(this, kConnectionType_Note));
   mAllNotesCable->GetPatchCableSource()->SetOverrideCableDir(ofVec2f(1, 0), PatchCableSource::Side::kRight);
   AddPatchCableSource(mAllNotesCable->GetPatchCableSource());
   mAllNotesCable->GetPatchCableSource()->SetManualPosition(318, 28);
}
...
void TuringSequencer::SetMidiController(std::string name)
{
   if (mController)
...
void TuringSequencer::DrawModule()
{
   ...
   mLengthSlider->Draw();
   mOctaveLoSlider->Draw();
   mOctaveHiSlider->Draw();
   //   mOctaveSlider->Draw();
...
void TuringSequencer::OnClicked(float x, float y, bool right)
{
...
void TuringSequencer::MouseReleased()
{
   IDrawableModule::MouseReleased();
   mGrid->MouseReleased();
}

bool TuringSequencer::MouseMoved(float x, float y)
{
...
bool TuringSequencer::MouseScrolled(float x, float y, float scrollX, float scrollY, bool isSmoothScroll, bool isInvertedScroll)
{
   mGrid->NotifyMouseScrolled(x, y, scrollX, scrollY, isSmoothScroll, isInvertedScroll);
   return false;
}

void TuringSequencer::SetEnabled(bool on)
{
   mEnabled = on;
   if (!on)
...
void TuringSequencer::CheckboxUpdated(Checkbox* checkbox, double time)
{
   if (checkbox == mEnabledCheckbox)
      mNoteOutput.Flush(time);
}

void TuringSequencer::GridUpdated(UIGrid* grid, int col, int row, float value, float oldValue)
{
   if (grid == mGrid)
   {
      for (int i = 0; i < mGrid->GetCols(); ++i)
...
int TuringSequencer::RowToPitch(int row)
{
   row += mRowOffset;
...
int TuringSequencer::PitchToRow(int pitch)
{
   for (int i = 0; i < mGrid->GetRows(); ++i)
   {
...
void TuringSequencer::SetStep(int index, int row, int velocity, float length)
{
   if (index >= 0 && index < TM_MAX_STEPS)
   {
...
void TuringSequencer::SetPitch(int index, int pitch, int velocity, float length)
{
   if (index >= 0 && index < TM_MAX_STEPS)
   {
...
void TuringSequencer::GetPush2Layout(int& sequenceRows, int& pitchCols, int& pitchRows)
{
...
bool TuringSequencer::OnPush2Control(Push2Control* push2, MidiMessageType type, int controlIndex, float midiValue)
{
...
void TuringSequencer::UpdatePush2Leds(Push2Control* push2)
{
   int sequenceRows, pitchCols, pitchRows;
   GetPush2Layout(sequenceRows, pitchCols, pitchRows);
...
void TuringSequencer::OnTransportAdvanced(float amount)
{
   PROFILER(TuringSequencer);
...
void TuringSequencer::OnPulse(double time, float velocity, int flags)
{
...
void TuringSequencer::OnTimeEvent(double time)
{
   if (!mHasExternalPulseSource)
      Step(time, 1, 0);
}

void TuringSequencer::Step(double time, float velocity, int pulseFlags)
{
   if (!mEnabled)
      return;
...
void TuringSequencer::SendNoteToCable(int index, double time, int pitch, int velocity)
{
   mStepCables[index]->PlayNoteOutput(time, pitch, velocity);
}

void TuringSequencer::UpdateLights()
{
   if (mController)
   {
      for (int i = 0; i < TM_MAX_STEPS; ++i)
...
int TuringSequencer::ButtonToStep(int button)
{
...
int TuringSequencer::StepToButton(int step)
{
   return step < 4 ? (step + 9) : (step + 21);
}

void TuringSequencer::GetModuleDimensions(float& width, float& height)
{
   width = 330;
   height = 65;
}

void TuringSequencer::UpdateVelocityGridPos()
{
   float gridX, gridY;
   mGrid->GetPosition(gridX, gridY, true);
}

void TuringSequencer::OnMidiNote(MidiNote& note)
{
...
void TuringSequencer::OnMidiControl(MidiControl& control)
{
...
void TuringSequencer::PlayNote(double time, int pitch, int velocity, int voiceIdx, ModulationParameters modulation)
{
   if (velocity > 0)
   {
      mHasExternalPulseSource = true;
...
void TuringSequencer::ShiftSteps(int amount)
{
   int newTones[TM_MAX_STEPS];
   int newVels[TM_MAX_STEPS];
   float newLengths[TM_MAX_STEPS];
   memcpy(newTones, mTones, TM_MAX_STEPS * sizeof(int));
   memcpy(newVels, mVels, TM_MAX_STEPS * sizeof(int));
   memcpy(newLengths, mNoteLengths, TM_MAX_STEPS * sizeof(float));
   ...
   memcpy(mTones, newTones, TM_MAX_STEPS * sizeof(int));
   memcpy(mVels, newVels, TM_MAX_STEPS * sizeof(int));
   memcpy(mNoteLengths, newLengths, TM_MAX_STEPS * sizeof(float));
   SyncGridToSeq();
}

void TuringSequencer::UpdateGridControllerLights(bool force)
{
...
void TuringSequencer::OnControllerPageSelected()
{
   UpdateGridControllerLights(true);
}

void TuringSequencer::OnGridButton(int x, int y, float velocity, IGridController* grid)
{
   int col = x + mGridControlOffsetX;
   int row = y - mGridControlOffsetY;
...
void TuringSequencer::ButtonClicked(ClickButton* button, double time)
{
   ...
   if (button == mSetStepToZeroButton)
   {
      RandomizePitches(GetKeyModifiers() & kModifier_Shift);
      SyncGridToSeq();
   }
   if (button == mSetStepToZeroButton)
   {
      SetStepToValue(false);
      SyncGridToSeq();
   }
   if (button == mSetStepToOneButton)
   {
      SetStepToValue(true);
      SyncGridToSeq();
   }
...
void TuringSequencer::RandomizePitches(bool fifths)
{
   if (fifths)
   {
      for (int i = 0; i < mLength; ++i)
      {
         if (ofRandom(1) <= mRandomizeStepChance)
         {
            switch (gRandom() % 5)
            {
               default:
...
void TuringSequencer::SetStepToValue(bool value)
{
   // Set current step to 0 or 1
}

void TuringSequencer::DropdownUpdated(DropdownList* list, int oldVal, double time)
{
...
void TuringSequencer::FloatSliderUpdated(FloatSlider* slider, float oldVal, double time)
{
   for (int i = 0; i < TM_MAX_STEPS; ++i)
   {
...
void TuringSequencer::SyncGridToSeq()
{
   mGrid->Clear();
   for (int i = 0; i < TM_MAX_STEPS; ++i)
   {
      ...
      mGrid->SetVal(i, mTones[i], mVels[i] > 0 ? 1 : 0, false);
   }
   mGrid->SetGrid(mLength, mNoteRange);
   if (mGridControlTarget->GetGridController())
   {
...
void TuringSequencer::OnScaleChanged()
{
   SetUpStepControls();
}

void TuringSequencer::SetUpStepControls()
{
...
void TuringSequencer::SaveLayout(ofxJSONElement& moduleInfo)
{
}

void TuringSequencer::LoadLayout(const ofxJSONElement& moduleInfo)
{
...
//   mModuleSaveData.LoadInt("gridsteps", moduleInfo, 8, 1, TM_MAX_STEPS, K(isTextField));
...
void TuringSequencer::SetUpFromSaveData()
{
...
//   UpdateVelocityGridPos();
   SyncGridToSeq();
   SetUpStepControls();
//   mGrid->SetClickValueSubdivisions(1);
}

void TuringSequencer::SaveState(FileStreamOut& out)
{
...
void TuringSequencer::LoadState(FileStreamIn& in, int rev)
{
