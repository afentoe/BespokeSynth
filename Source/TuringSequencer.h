...
public:
   TuringSequencer();
   virtual ~TuringSequencer();
   static IDrawableModule* Create() { return new TuringSequencer(); }
...
private:
   ...
   void SetStepToValue(bool value);
   ...
   int mTones[TM_MAX_STEPS]{};
   int mVels[TM_MAX_STEPS]{};
   float mNoteLengths[TM_MAX_STEPS]{};
   ...
   ClickButton* mSetStepToZeroButton{ nullptr };
   ClickButton* mSetStepToOneButton{ nullptr };
   float mRandomizeStepChance{ 1 };
   FloatSlider* mRandomizeStepChanceSlider{ nullptr };
   float mOctaveLo{ 2 };
   FloatSlider* mOctaveLoSlider{ nullptr };
   float mOctaveHi{ 4 };
   FloatSlider* mOctaveHiSlider{ nullptr };

   std::array<double, TM_MAX_STEPS> mLastStepPlayTime{ -1 };
   std::array<DropdownList*, TM_MAX_STEPS> mToneDropdowns;
   std::array<IntSlider*, TM_MAX_STEPS> mVelocitySliders;
   std::array<FloatSlider*, TM_MAX_STEPS> mLengthSliders;

   bool mHasExternalPulseSource{ false };

   AdditionalNoteCable* mAllNotesCable{ nullptr };

   std::array<AdditionalNoteCable*, TM_MAX_STEPS> mStepCables;
